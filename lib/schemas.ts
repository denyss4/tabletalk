...
export const receiptSchema = z.object({
 ...
 subtotalMinor:minor.nullable(),serviceMinor:minor.nullable(),totalMinor:minor.nullable(),warnings:z.array(z.string().max(300)).max(10)
}).strict();
...
export const receiptJsonSchema=object({merchant:str,currency:str,rows:{type:'array',items:object({name:str,quantity:{type:'integer'},amountMinor:amount,uncertain:{type:'boolean'}})},subtotalMinor:amount,serviceMinor:amount,totalMinor:amount,warnings:strings});
