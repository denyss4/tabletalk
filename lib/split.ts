...
export function apportion(total: number, weights: number[]): number[] {
  ...
  const order = weights.map((w, i) => ({ i, rem: BigInt(total) * BigInt(w) % sum })).sort((a, b) => a.rem === b.rem ? a.i - b.i : a.rem > b.rem ? -1 : 1);
...
export function validateReceipt(r: Receipt): void {
  if (r.currency !== 'EUR') throw new Error('This prototype supports EUR receipts. Please use a EUR receipt.');
  if (!Array.isArray(r.rows) || r.rows.length < 1 || r.rows.length > 10) throw new Error('Use a receipt with one to ten item rows.');
  if (new Set(r.rows.map(x => x.id)).size !== r.rows.length) throw new Error('Duplicate receipt row.');
...
export function itemsOf(receipt: Receipt): Item[] {
  ...
  const all = receipt.rows.flatMap(row => {
    const amounts = row.amountMinor === null ? Array(row.quantity).fill(null) : apportion(row.amountMinor, Array(row.quantity).fill(1));
    return amounts.map((amountMinor, index) => ({ id: `${row.id}.${index + 1}`, rowId: row.id, name: row.name, amountMinor, uncertain: row.uncertain }));
  });
  ...
  return all.map(item => {
    ...
    return { ...item, name: repeat ? `${item.name} #${counts.get(key)}` : item.name };
  });
}
export function applyAssignments(state: SplitState, assignments: Assignment[]): SplitState {
  ...
  for (const assignment of assignments) {
    ...
    for (const id of assignment.itemIds) {
      ...
      touched.add(id);
      next[id] = state.people.filter(p => assignment.personIds.includes(p.id)).map(p => p.id);
    }
  }
  if (Object.values(next).filter(owners => owners.length > 1).length > 1) throw new Error('This prototype supports one shared item per receipt.');
...
export function calculate(state: SplitState) {
  ...
  let assigned = 0;
  for (const id of Object.keys(state.allocation)) if (!knownIds.has(id)) issues.push('Allocation contains an unknown item.');
  for (const item of items) {
    const owners = state.allocation[item.id] ?? [];
    if (!owners.length) continue;
    if (new Set(owners).size !== owners.length || owners.some(id => !state.people.some(p => p.id === id))) { issues.push(`Check owners of ${item.name}.`); continue; }
    assigned++;
    if (item.amountMinor !== null) {
      const shares = apportion(item.amountMinor, state.people.map(p => owners.includes(p.id) ? 1 : 0));
      shares.forEach((n, i) => { subtotals[i] += n; });
    }
  }
  ...
  if (assigned !== items.length) issues.push(`${items.length - assigned} item${items.length - assigned === 1 ? '' : 's'} still need an owner.`);
  ...
  const service = settled ? apportion(r.serviceMinor!, subtotals) : state.people.map(() => 0);
  const people = state.people.map((p, i) => ({ ...p, subtotal: subtotals[i], service: service[i], total: subtotals[i] + service[i] }));
  const sum = people.reduce((n, p) => n + p.total, 0);
  if (settled && sum !== r.totalMinor) throw new Error('Balance verification failed.');
  return { settled, people, assigned, itemCount: items.length, issues: [...new Set(issues)], subtotal, sum };
}
