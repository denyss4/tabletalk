...
export function useTabletalk(){
 ...
 const [speechMode,setSpeechMode]=useState<'browser'|'provider'>('provider');const [speechPreview,setSpeechPreview]=useState('');const speech=useRef<ReturnType<typeof listenForSpeech>|null>(null);
 const [pendingPhoto,setPendingPhoto]=useState<PhotoInput|null>(null);
 const [checkingConnection,setCheckingConnection]=useState(false);const connectionRequest=useRef<Promise<boolean>|null>(null);
 const [requestingMic,setRequestingMic]=useState(false);const micRequest=useRef(false);const mounted=useRef(true);
 ...
 const result=state?calculate(state):null;const settled=!!result?.settled&&!pending&&!confirmation&&!busy&&!recording&&!requestingMic;
 ...
 const checkConnection=useCallback(async()=>{
  ...
  const request=(async()=>{try{const response=await fetch('/api/status',{cache:'no-store',signal:AbortSignal.timeout(8000)});if(!response.ok)throw new Error('Connection check failed');const data=await response.json() as {configured?:unknown;speechMode?:string};setSpeechMode(data.speechMode==='browser'?'browser':'provider');if(typeof data.configured!=='boolean')throw new Error('Invalid status');setConfigured(data.configured);return data.configured as boolean;}catch{setConfigured(null);return false;}finally{setCheckingConnection(false);connectionRequest.current=null;}})();
  connectionRequest.current=request;return request;
 },[]);
 useEffect(()=>{mounted.current=true;void checkConnection();const refresh=()=>{void checkConnection();};window.addEventListener('focus',refresh);return()=>{mounted.current=false;speech.current?.abort();window.removeEventListener('focus',refresh);if(recordingTimer.current)clearTimeout(recordingTimer.current);if(recorder.current?.state==='recording'){recorder.current.onstop=null;recorder.current.stop();}streamRef.current?.getTracks().forEach(t=>t.stop());recordingUrls.current.forEach(url=>URL.revokeObjectURL(url));};},[checkConnection]);
 ...
 function clearSession(){stateRef.current=null;setState(null);setPhoto('');setPendingPhoto(null);setSpeechPreview('');setExample(false);setPending(null);setConfirmation(null);setOperations([]);setActivity([]);setHistory([]);setError('');setSettledMs(null);setFirstSettledMs(null);startedAt.current=null;recordingUrls.current.forEach(url=>URL.revokeObjectURL(url));recordingUrls.current=[];setRecordings([]);}
 function reset(){if(busyRef.current||recording||micRequest.current)return;clearSession();}
 function startExample(){if(busyRef.current||recording||micRequest.current)return;reset();startedAt.current=Date.now();setExample(true);setPhoto('/samples/receipt-display.webp');const next={receipt:structuredClone(sampleReceipt),people:structuredClone(people),allocation:{},revision:0};stateRef.current=next;setState(next);note('Opened the allocation example. Photo recognition was not run.');}
 async function preparePhoto(file:File){
  ...
  let image:string;try{image=await resizeImage(file);}catch{throw new Error('That image could not be opened. Try a different JPG, PNG or WebP photo.');}
  const input={image,name:file.name};clearSession();startedAt.current=Date.now();setPendingPhoto(input);setPhoto(image);await recognizePhoto(input);
 }
 ...
 async function readTemplate(template:ReceiptTemplate){
  ...
  await run('Reading your receipt...',async()=>{
   let response:Response;try{response=await fetch(template.src,{signal:AbortSignal.timeout(15_000)});}catch{throw new Error('The template photo could not be loaded. Choose it again to retry.');}
   if(!response.ok)throw new Error('The template photo is unavailable. Please choose another receipt.');
   await preparePhoto(new File([await response.blob()],template.src.split('/').pop()!,{type:template.type}));
  });
 }
 async function recognizePhoto(input:PhotoInput){
  if(!await checkConnection())throw new Error('Your photo is ready. Connect the recognition service, then choose Retry recognition.');
  const response=await api('/api/receipt',JSON.stringify({image:input.image}),{'Content-Type':'application/json'});
  const next={receipt:response.receipt,people:structuredClone(people),allocation:{},revision:0};calculate(next);stateRef.current=next;setState(next);setPendingPhoto(null);note('Receipt read. Check the rows, then tell us who had what.');
 }
 async function retryPhoto(){if(pendingPhoto)await run('Reading your receipt…',()=>recognizePhoto(pendingPhoto));}
 async function readSample(unreadable=false){try{const response=await fetch(`/samples/receipt${unreadable?'-unreadable':''}.jpg`);if(!response.ok)throw new Error('Sample photo could not be loaded.');await uploadPhoto(new File([await response.blob()],'sample.jpg',{type:'image/jpeg'}));}catch(e){setError(e instanceof Error?e.message:'Sample unavailable.');}}
 ...
 function confirmReceiptAmount(field:'subtotalMinor'|'serviceMinor'|'totalMinor',amountMinor:number,description:string){if(!stateRef.current||busyRef.current||recording)return;try{commit(confirmReceipt(stateRef.current,{field,rowId:null,amountMinor,name:null}));note(description,'change');setError('');}catch(e){setError(e instanceof Error?e.message:'Please try again.');}}
 ...
 function applyVoiceResult(data:{revision:number;transcript:string;intent:Intent}){
 ...
 async function sendTranscript(transcript:string){const snapshot=stateRef.current;if(!snapshot)return;await run('Interpreting your command...',async()=>{const data=await api('/api/voice',JSON.stringify({state:snapshot,transcript,pending}),{'Content-Type':'application/json'});applyVoiceResult(data);});}
 function startBrowserRecording(){
  const Constructor=speechConstructor();if(!Constructor){setError('Speech recognition is not available in this browser. Open TableTalk in a browser with speech recognition, such as Chrome, or use the row controls.');return;}
  const started=Date.now();setError('');setSpeechPreview('');setRecording(true);
  const log=(status:string)=>setOperations(o=>[...o,{id:crypto.randomUUID(),stage:'transcription',model:'browser SpeechRecognition',provider:'browser speech service',latencyMs:Date.now()-started,audioSeconds:(Date.now()-started)/1000,status,costUSD:null,costBasis:'Browser-managed speech recognition. Service pricing unverified; no app API transcription charge measured.'}]);
  const finish=()=>{if(recordingTimer.current)clearTimeout(recordingTimer.current);speech.current=null;setRecording(false);};
  try{speech.current=listenForSpeech(new Constructor(),{onPreview:setSpeechPreview,onComplete:transcript=>{finish();log('succeeded');void sendTranscript(transcript);},onError:message=>{const current=speech.current;finish();current?.abort();log('failed');setError(message);}});recordingTimer.current=setTimeout(()=>speech.current?.stop(),60_000);}catch{finish();setError('Speech recognition could not start. Check microphone permissions and try again.');}
 }
 async function toggleRecording(){
  if(recording&&speech.current){speech.current.stop();return;}
  ...
  if(speechMode==='browser'){startBrowserRecording();return;}
  ...
  micRequest.current=true;setRequestingMic(true);
  try{
   setError('');if(!await checkConnection()){setError('Voice recognition needs a server connection. Connect the service, check the connection, then record again.');return;}
   const stream=await navigator.mediaDevices.getUserMedia({audio:true});if(!mounted.current){stream.getTracks().forEach(t=>t.stop());return;}streamRef.current=stream;
   const mime=['audio/webm;codecs=opus','audio/mp4','audio/webm'].find(x=>MediaRecorder.isTypeSupported(x));const rec=new MediaRecorder(stream,mime?{mimeType:mime}:undefined);recorder.current=rec;const chunks:BlobPart[]=[];const start=Date.now();
   rec.ondataavailable=e=>{if(e.data.size)chunks.push(e.data);};
   rec.onstop=()=>{if(recordingTimer.current)clearTimeout(recordingTimer.current);stream.getTracks().forEach(t=>t.stop());streamRef.current=null;setRecording(false);const duration=(Date.now()-start)/1000;const type=rec.mimeType||'audio/webm';const blob=new Blob(chunks,{type});if(blob.size<100||duration<0.3){setError('No recording was captured. Hold the microphone open long enough to say your command.');return;}void sendAudio(blob,`voice-${Date.now()}.${type.includes('mp4')?'m4a':'webm'}`,duration);};
   rec.onerror=()=>{rec.onstop=null;if(recordingTimer.current)clearTimeout(recordingTimer.current);stream.getTracks().forEach(t=>t.stop());streamRef.current=null;setRecording(false);setError('Recording failed. Try uploading an audio file.');};
   rec.start();setRecording(true);recordingTimer.current=setTimeout(()=>{if(rec.state==='recording')rec.stop();},60_000);
  }catch{streamRef.current?.getTracks().forEach(t=>t.stop());streamRef.current=null;setError('Microphone access was not granted or the microphone is unavailable. Check browser permissions, or upload an audio recording.');}finally{micRequest.current=false;setRequestingMic(false);}
 }
 async function uploadAudio(file:File){if(speechMode==='browser'){setError('Recording uploads are not supported by this provider. Use Start voice command.');return;}if(busyRef.current||recording||micRequest.current||!stateRef.current)return;if(file.size>10_000_000){setError('Use an audio recording under 10 MB.');return;}try{const context=new AudioContext();let duration:number;try{duration=(await context.decodeAudioData(await file.arrayBuffer())).duration;}finally{await context.close();}if(duration>90)throw new Error('Keep the recording under 90 seconds.');await sendAudio(file,file.name,duration);}catch(e){setError(e instanceof Error?e.message:'Could not read that audio file. Try WAV, MP3 or WebM.');}}
 ...
 function exportEvidence(){const payload={exportedAt:new Date().toISOString(),inputMode:example?'prefilled allocation example; no photo recognition':'photo recognition',state,verification:result,settled,unresolvedQuestion:pending,unconfirmedReceiptEdit:confirmation,timing:{startAt:startedAt.current?new Date(startedAt.current).toISOString():null,firstSettledMs,lastSettledMs:settledMs,includesUserThinkingAndRecording:true},operations,activity,estimatedKnownCostUSD:operations.reduce((n,o)=>n+(o.costUSD??0),0),costIncomplete:operations.some(o=>o.costUSD===null),pricingDate:'2026-09-18',hosting:'Excluded; report actual host plan separately',speechOutput:'No generated speech; clarification choices only',paidIntermediaries:[...new Set(operations.map(o=>o.provider).filter(p=>p&&p!=='https://api.openai.com'))],speechRecognitionMode:speechMode,recordings:recordings.map(({name,duration})=>({name,duration}))};download(JSON.stringify(payload,null,2),'tabletalk-evidence.json','application/json');}
 return {state,people,photo,example,busy,error,configured,speechMode,speechPreview,pendingPhoto,retryPhoto,checkingConnection,checkConnection,requestingMic,pending,confirmation,operations,activity,recording,recordings,result,settled,settledMs,firstSettledMs,history,reset,startExample,uploadPhoto,readSample,readTemplate,assign,resolveCandidate,acceptConfirmation,confirmVisibleRow,confirmReceiptAmount,undo,rename,toggleRecording,uploadAudio,useSampleAudio,exportEvidence,dismissPending:()=>{setPending(null);setConfirmation(null);note('Discarded the unresolved change. Existing allocations kept.','change');}};
}
