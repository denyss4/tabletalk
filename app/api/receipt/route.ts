...
export async function POST(request:Request){const operations:Operation[]=[];try{
 ...
 const raw=await structured(instructions,[{role:'user',content:[{type:'input_text',text:'Read this receipt faithfully. Distinguish details that are absent from details that are present but unreadable.'},{type:'input_image',image_url:image,detail:'high'}]}],receiptJsonSchema,'receipt',operations);
