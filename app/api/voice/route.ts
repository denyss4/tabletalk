...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (Number(request.headers.get("content-length") ?? 0) > 12_000_000)
      throw new AppError("Use a recording shorter than one minute.", 413);
    let state: SplitState;
    let pendingIntent: ReturnType<typeof intentSchema.parse> | null = null;
    let pending: string;
    let transcript: string;
    if (request.headers.get("content-type")?.includes("application/json")) {
      if (SPEECH_MODE !== "browser")
        throw new AppError(
          "Upload a voice recording for this recognition service.",
        );
      const input = z
        .object({
          state: stateSchema,
          transcript: z.string().trim().min(1).max(4000),
          pending: intentSchema.nullable().optional(),
        })
        .strict()
        .parse(await request.json());
      state = input.state;
      pendingIntent = input.pending ?? null;
      pending = JSON.stringify(pendingIntent);
      transcript = input.transcript;
      validateReceipt(state.receipt);
    } else {
      if (SPEECH_MODE === "browser")
        throw new AppError(
          "This provider has no recording transcription model. Use Start voice command in a browser with speech recognition.",
          422,
      ...
      if (
        ...
        throw new AppError("Choose a valid audio recording under 10 MB.");
      state = stateSchema.parse(JSON.parse(String(form.get("state") ?? "")));
      validateReceipt(state.receipt);
      const duration = Number(form.get("duration"));
      if (!Number.isFinite(duration) || duration <= 0 || duration > 90)
        throw new AppError("Use a recording of 90 seconds or less.");
      pending = String(form.get("pending") ?? "").slice(0, 4000);
      try {
        pendingIntent = intentSchema
          .nullable()
          .parse(JSON.parse(pending || "null"));
      } catch {
        throw new AppError(
          "The pending clarification could not be validated. Please try again.",
        );
      }
      const audioForm = new FormData();
      audioForm.append("file", audio, audio.name);
      audioForm.append("model", TRANSCRIBE_MODEL);
      audioForm.append("language", "en");
      audioForm.append("response_format", "json");
      audioForm.append(
        "prompt",
        `English restaurant order allocation. Names: ${state.people.map((p) => p.name).join(", ")}. Items: ${state.receipt.rows.map((r) => r.name).join(", ")}.`,
      );
      const recognized = await provider(
      ...
      if (typeof recognized.text !== "string" || !recognized.text.trim())
        throw new AppError("No speech was recognized. Please record again.");
